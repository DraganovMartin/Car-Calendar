    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        saveBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(userManager.validateRegister(emailET.getText().toString(),passET.getText().toString())){
                    int age = Integer.parseInt(ageET.getText().toString());
                    if(age < 16){
                        ageET.setError("You must be 16 and above");
                        return;
                    }
                    userManager.registerUser(userManager.createUser(usernameET.getText().toString(),passET.getText().toString(),age,emailET.getText().toString()));
                    Intent dataToLogin = new Intent();
                    dataToLogin.putExtra("Email",emailET.getText().toString());
                    dataToLogin.putExtra("Password",passET.getText().toString());
                    setResult(LoginActivity.DATA_OKEY,dataToLogin);
                    finish();
                }
                else{
                    Toast.makeText(view.getContext(),"Wrong e-mail or weak password !",Toast.LENGTH_SHORT).show();
                }
            }
        });
